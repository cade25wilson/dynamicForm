<template>
    <div>
        <div class="mt-6">
            <div class="w-full">
                <div>
                    Your credit card will be charged: ${{ formSection.options.amount }}<br>
...
const handlePayment = async () => {
  ...
  try {
    ...
    if (error) {
      ...
      emit('updateResponse', { response, response });
    }
  } catch (err) {
...
const fetchClientSecret = async () => {
  try {
    ...
    const response = await fetch(`/payment-intent/${props.formSection.id}`, {
      method: 'POST',
      headers: { 
