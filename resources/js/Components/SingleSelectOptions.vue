            ...
            <div>
                <ul class="space-y-2 mt-2">
                    <li v-for="(option, index) in page.props.current_section.form_fields[0].options.choices" :key="index" @click.outside="showSetting[index] = false" @image-uploaded-to-owner.window="handleImageUploaded" class="flex items-center bg-white relative">
                        ...
                        <button v-if="page.props.current_section.section_type_id != 10" type="button" title="option settings" @click.stop="toggleShowSetting(index)" class="border border-gray-300 border-l-0 shadow-sm py-2.5 px-2 rounded-md rounded-l-none">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                        ...
                        <button v-else @click="deleteOption(index)" type="button" class="border border-gray-300 border-l-0 shadow-sm py-2.5 px-2 rounded-md rounded-l-none">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                        ...
                        <!-- Settings Dropdown -->
                        <div class="absolute right-0 z-10 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" v-if="showSetting[index] && page.props.current_section.section_type_id != 10">
